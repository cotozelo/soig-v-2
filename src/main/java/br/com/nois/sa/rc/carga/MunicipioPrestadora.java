...
public class MunicipioPrestadora {
	...
	public void run(String caminho) {
		 ...
		 * this.versaoRepository); List<AgenciaTO> agencias =
		 * agenciaController.getAll();
		 * 
		 * AgenciaTO agencia; if (agencias.size() > 0) { agencia =
		 * agencias.get(0); } else { agencia = new AgenciaTO();
		 * agencia.setAtivo(true); agencia.setNome("Agencia Completa"); agencia
		 * = agenciaController.insert(agencia); } String idAgencia =
		 * agencia.getId();
		 * 
		 ...
		 * 
		 * for (String linha : linhas) { // 0 ;1 ;2 ;3 ;4 ;5 ;6 ;7 ;8 //
		 * codmun;municipio;UF;codpre;prestadora;sigla;abrangencia;natureza;
		 * servico String[] linhaSplit = linha.split(";");
		 * 
		 ...
		 * if (municipio == null) { municipio = new Municipio(); } else {
		 * municipio.setAtiva(true); } if (municipio.getId() == null) {
		 ...
		 * municipio.setAgenciaId(idAgencia);
		 * 
		 * municipio = municipioController.insert(municipio); if
		 * (municipio.getError() != null) { erroMun.add(linha); } }
		 * 
		 * if (linhaSplit.length > 8) { Prestadora prestadora =
		 * prestadoraController.getIdCodigo(municipio.getId(),
		 * linhaSplit[3].trim());
		 * 
		 ...
		 * prestadora); if (prestadora.getError() != null) { erroMun.add(linha);
		 * } else { System.out.println("[NOVO]" + linha); } } } } }
		 * 
		 * if (!erroMun.isEmpty()) { System.out.println("Totado de erros " +
		 * erroMun.size()); for (String erro : erroMun) {
