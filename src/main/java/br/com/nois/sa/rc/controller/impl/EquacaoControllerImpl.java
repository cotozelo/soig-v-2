...
public class EquacaoControllerImpl implements EquacaoController {
	...
	public EquacaoControllerImpl(IndicadorRepository indicadorRepository, LogRepository logRepository, VersaoRepository versaoRepository) {
	...
	public Map<String, String> getIndicadoresByDado(String dado, String ano) {
		Map<String, String> equacoes = new HashMap<String, String>();

		for (IndicadorTO indicadorTO : this.indicadorRepository.findAll()) {
			if (indicadorTO.getEquacaoAtiva() != null && !indicadorTO.getEquacaoAtiva().isEmpty()) {
				for (EquacaoTO equacaoTO : indicadorTO.getEquacaoAtiva()) {
					if(equacaoTO.getFormula() != null && equacaoTO.getFormula().toUpperCase().contains(dado.toUpperCase()) &&  equacaoTO.getAno().toUpperCase().contains(ano.toUpperCase())) {
						equacoes.put(indicadorTO.getSigla(), equacaoTO.getFormula());
					}
				}
			}
		}
